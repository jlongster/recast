...
function genericPrintNoParens(path, options, print) {
    ...
    case "ObjectTypeAnnotation":
        ...
        fields.forEach(function(field) {
            path.each(function(childPath) {
                props.push(print(childPath));
            }, field);
        });

        var trailingComma = util.isTrailingCommaEnabled(options, "objects") ? "," : "";
        var joined = fromString(", ").join(props);
        if(joined.getLineLength(1) > options.wrapColumn || joined.length > 1) {
            ...
            var space = "";
            if (options.objectCurlySpacing) {
                space = " ";
            }
            return concat([
                leftBrace + space, joined, trailingComma, space + rightBrace
            ]);
        }
...
function printArgumentsList(path, options, print) {
    ...
    var lastArg = printed.length && printed[printed.length - 1];
    var joined = fromString(", ").join(printed);
    if (joined.getLineLength(1) > options.wrapColumn ||
        (lastArg && joined.length !== lastArg.length)) {
        joined = fromString(",\n").join(printed);
        return concat([
            "(\n",
